...
function CherryService(){
	...
	//---------------------------------------------------
	{
		...
		this.GetPlaylistList_Home = async function(country_code){
			return new Promise(async function(resolve, reject){
				var conn = null;
				try{
					conn = await db_conn.GetConnection();
					...
					conn.query(sql, val, function(err, result){
						if(err){
							console.error(err);
							reject('FAIL CherryService GetPlaylistList_Home #0');
						}else{
							resolve(result);
						}
					});
					...
					console.error(err);
					reject('FAIL CherryService GetPlaylistList_Home #1');
				}finally{
					if(conn) conn.release();
				}
			});
		...
		this.GetPlaylistMusicList = async function(playlist_uid, user_id){
			return new Promise(async function(resolve, reject){
				...
				try{
					conn = await db_conn.GetConnection();
					var sql = `
						SELECT pm.sort, pm.music_uid, a.name AS artist, a.artist_uid, a.is_various, m.title, m.video_id, u.name user_name,
							concat('[',v.member_list_json,']') as member_list_json,
							IF(lm.user_id IS NULL, 'N', 'Y') as is_like
						FROM playlist_music pm
						JOIN music m ON m.music_uid=pm.music_uid
						JOIN artist a ON m.artist_uid=a.artist_uid
						JOIN user u ON m.user_id = u.user_id
						LEFT JOIN va_member_view as v ON a.artist_uid=v.artist_uid
						LEFT JOIN (
			...
			return new Promise(async function(resolve, reject){
				...
				try{
					...
					conn.query(sql, val, function(err, result){
						if(err){
							console.error(err);
							reject('FAIL CherryService ERA_GetMusicList #0');
						}else{
							resolve(result);
						}
					});
					...
					console.error(err);
					reject('FAIL CherryService ERA_GetMusicList #1');
				}finally{
					if(conn) conn.release();
				}
			});
